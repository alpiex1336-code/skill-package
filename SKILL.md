...
name: prompt-upgrade
description: >-
  **MANDATORY RUNTIME ORCHESTRATION:** For product-making/changing work, pass **NON-NEGOTIABLE EXECUTION GATES** before **Implement**-phase artifact writes—invoke **runtime subagents** via **Task tool** (or equivalent delegation) for **Orchestra wave**s; **N > 1** **composed role label**s per wave; **one runtime subagent per Work Filter–accepted Quintessence row** at Implement when tooling allows. **Simulated fallback** when Task/delegation is unavailable—not the default. **In-session wave ledger** required before Implement on thorough/deep-upgrade runs—not deferred to the final reply. **Execution gate** discipline applies default-on; map the same workflow to any artifact in scope (product, code, docs, policy, research, planning, governance, or this skill package). **Default-on** means use for every turn; scale orchestra depth with **`improvement_mode`** and user scope—skip full orchestration only when the user explicitly requests a trivial single-step action where multi-role review adds no value. **Improvement mode selection:** resolve by intent before orchestration: when the user clearly signals **normal** (small fix, small talk, single concern) or **thorough** / **deep-upgrade** (whole-product, all-aspects, maximum optimization), choose that mode and scale the workflow accordingly; when product-making/changing intent is present and the mode cannot be identified, **ask once and wait** for **normal** vs **thorough**. **Deep-upgrade intent** pins **thorough** and the **Deep-upgrade procedural tier**. **Quintessence pipeline:** sample many composed roles across waves, **integrate** overlapping ideas and roles into **`theme_key`** quintessence rows, adopt via Work Filter, then **implement/verify** preferably with **subagents per quintessence theme**. Coordinates many subagents per run using composed two-part roles from the word banks (`[Slot A][Slot B]`); see **Execution contract** and [reference-execution-gates.md](reference-execution-gates.md). **Cursor / LLM skill:** same **Stochastic Role Orchestra** pattern for any assistant that reads this file. Expects **universal aspect-coverage** discipline—build a **target coverage set** per engagement (see **Lexicon**), then **bias** role draws toward **uncovered** aspect classes. Uses **word-bank priority tiers (P0 / P1 / P2)** so **`improvement_mode` = thorough** whole-product runs have a **session floor** of **P0 families** (see [reference-word-banks.md](reference-word-banks.md)). **`normal`** biases **incremental modification**; **`thorough`** authorizes **material reshaping** while preserving **north-star invariant**. Requires epistemic honesty (no invented context; anti-sycophancy). **Outward closure:** after honest saturation (**Workflow §8**), terminal replies stay decisive. **Voice:** professional register; avoid LLM-default “AI voice.” **Skill-targeting requests** use **Recursive self-application**. Triggers include “essential fixes only,” “surgical,” “full product pass,” “**thorough** pass,” “multi-agent review,” “orchestrated review,” **mature product iteration**, greenfield/brownfield maturity passes. **Supersedes** informal fixed job-title maturity loops by mapping concerns onto **composed roles** + depth references—stricter anti–single-voice discipline.
---
...
- [ ] **Runtime-first probe** — If Task tool exists, **≥1 Orchestra wave** invoked **runtime subagents**. If unavailable, wave ledger records `runtime_subagents: unavailable (<reason>)` and **Simulated fallback** runs instead.
- [ ] **Wave ledger started in-session** — Wave 1 row exists **before Implement** (`wave_id`, labels, `theme_key`s, adoption notes)—**not** deferred to the closing reply.
...
No in-scope artifact file edits from raw role chatter. Adopted work flows only through **Quintessence row**s after **integrate → adopt** (Work Filter).
...
Each wave records `wave_id`, sampled labels, merged `theme_key`s, adoption notes, verification pointer, and skipped/substitute obligations—or honestly states simulated fallback and why. Thorough / deep-upgrade runs with **no ledger when Implement begins** are **incomplete**—stop and run orchestra first.
...
2. **Deep-upgrade procedural tier** — Execute the obligations below as a minimum procedural floor. If honestly blocked by environment, tooling, or installed-skill availability, record what was skipped, why, the substitute path, and verification impact in the wave ledger (**Tool and skill availability (honesty)**); disclosure is not completion. Continue the workflow with available substitutes and recurse through orchestra waves until **Workflow §8** and the **Stop rule for orchestra waves** justify stopping.
...
| **B** | **Runtime orchestra + labels** | Use **multiple runtime subagents** when available (Cursor **Task** tool or other agent delegation **is** an acceptable mechanism—not the only one). Each pass carries **`[Slot A][Slot B]`** (optional **`[Slot C]`** and/or **`[Slot D]`** per availability honesty rules). If runtime subagents **cannot** run, state the limitation in the ledger and run **simulated** orthogonal passes instead—still **N > 1** roles, P0 floor for thorough whole-product work, and additional waves until stop signals. **If Task exists and was not invoked, tier B is not satisfied.** |
| **C** | **Auditable wave ledger (in-session, not end-only)** | Start the ledger **before Implement**—as a visible section in the first reply that completes orchestra, or a maintained register artifact. **Per wave**, append at minimum: `wave_id`, sampled composed labels, merged `theme_key`s, brief adoption note per theme, verification pointer, plus skipped/substitute/residual-risk notes. **Final reply must not be the first appearance of the ledger** on thorough/deep-upgrade runs; if no wave rows exist when Implement begins, **stop and run orchestra first**. |
...
4. **Label discipline** — Every real subagent must carry and return one explicit composed label: **`[Slot A][Slot B]`** minimum; **`[Slot C]`** only when a tool/MCP/plugin token from the bank is **both** justified **and** verified available; **`[Slot D]`** only when an installed Agent Skill has been fully read this session, summarized as a short skill guideline, and is relevant to the pass—reject unlabelled or falsely-capable outputs for integration.

   **Runtime-first probe (mandatory at Orchestrate start):** Explicitly decide: **Task/subagent tooling available?** (Cursor Task tool, agent delegation—check environment, do not assume absence.)
   - **If YES:** First orchestra wave **must include ≥1 runtime subagent invocation** with the composed label in the prompt. Thorough greenfield/whole-product work: prefer **parallel Task calls** for independent aspect classes.
   - **If NO:** Record in ledger: `runtime_subagents: unavailable (<reason>)`; immediately run **simulated** fresh-agent passes for the same sampled labels—**same N, same integration discipline**. Simulated is **fallback**, not opt-out.
   - **Forbidden:** Treating “I can think in roles internally” as orchestra execution without labeled, separable pass outputs suitable for integration.

5. **Auditable waves (in-session)** — For each wave, record **before Implement** at minimum: `wave_id`, sampled labels, merged `theme_key`s, support count per theme, weighted adoption decision, and verification result. The terminal reply alone is **not** sufficient if no prior wave rows exist in-session.
...
**Runtime prompt invariant:** Prompts for review, implementation, verification, and acceptance subagents must include enough context for a fresh agent to succeed without guessing: purpose / why this subagent was launched; capability or domain boundary; compact **Snapshot** context packet; relevant skill instructions and companion references; **Slot D skill guideline(s)** when an installed skill is attached; constraints and non-goals; evidence hooks / verification expectation; and output contract. Keep templates and mechanics in [reference-execution-gates.md](reference-execution-gates.md).
...
### Single-voice collapse under delivery pressure

**Symptom:** Agent reads this skill, then **one generic voice** scopes, designs, and implements—no Task/subagent calls, no simulated role passes, no wave ledger until (maybe) the final summary.

**Why it violates the contract:** The skill replaces “one expert rotating hats” with **decorrelated passes** and **auditable integration**. Skipping orchestra to ship faster is **not** Work Filter efficiency.

**Common triggers:** greenfield urgency, empty repo, “just build it,” thorough/deep-upgrade phrasing treated as style not procedure.

**Required recovery (before any product edit):** (1) Stop implementation. (2) Run **Wave 1** with **N ≥ 2** roles (runtime first; simulated if Task blocked). (3) Write **ledger row 1**. (4) Resume only from **accepted quintessence** rows.
...
| # | Failure mode | Symptom | Hard stop rule | Self-check |
|---|--------------|---------|----------------|------------|
| **1** | **Single-voice collapse** | One generic expert voice; no composed labels on findings | **Do not implement** until **≥2 distinct labels** produced bounded tagged findings, **integrated** into `theme_key` rows | *Can I name two labels whose outputs changed what I adopted?* |
| **2** | **Skill-as-flavor-text** | Skill vocabulary in prose but **Understand → Implement** with no integrate/adopt | **Do not write files** until **PRE-FLIGHT** satisfied in session notes or draft ledger | *Does my plan still contain sample → integrate → adopt before implement?* |
...
Complete **all** items **in order** before the first **Write**, **StrReplace**, **EditNotebook**, or destructive shell mutation on product artifacts. Answer in one compact block (session notes, draft ledger, or reply preamble):

```
PRE-FLIGHT — product work gate
1. SCOPE     Artifact + audience from Snapshot. North-star invariant one line.
2. MODE      improvement_mode = normal | thorough (or scale-down if trivial single-step).
3. COVERAGE  Target coverage set. P0 families: instantiated | N/A + why.
4. ORCHESTRA Wave plan: N ≥ 2 composed labels (or single-axis exception cited).
             Task available? → runtime subagents first [Y/N + blocker if N].
...
| **Execution gate** | A pass/fail checkpoint (**Gate 0–4**, **Gate checklist**) that must be satisfied before the next workflow phase; distinct from Work Filter (what ships) and stop rules (when to end). See **NON-NEGOTIABLE EXECUTION GATES**. |
| **Subagent** | A reviewer pass bound to one **composed role label**. **Default:** real runtime subagent process when available; fallback: simulated pass when tooling is unavailable. See **Simulated fallback (execution sense)**. |
| **Simulated fallback (execution sense)** | Orthogonal composed-role passes **without** runtime Task/delegation—**only** when tooling is unavailable and **must** be disclosed in Gate 4 / wave ledger. Not “thinking in roles” in one voice. |
| **Composed role label** | At minimum `[Slot A…][Slot B…]` from the word banks; **optional** `[Slot C…]` names a **tool/MCP/plugin/hook** surface and **optional** `[Slot D…]` names an **installed Agent Skill** from [reference-word-banks.md](reference-word-banks.md). Optional slots apply **only** when availability and read-before-use rules are satisfied in the **Snapshot**—see **Tool and skill availability (honesty)**. Sole persona baggage for A/B; C/D are capability hints only. |
...
| **Slot D (optional)** | Fourth bracket group naming installed Cursor Agent Skills or skill packages whose full `SKILL.md` has been read in the current session and summarized into a short **skill guideline** (`use for` + `when to use`) before being attached. Slot D is an instruction/capability hint, not proof a tool ran and not a second controller. |
| **Skill guideline** | Short local note required before using Slot D: what the installed skill is for and when it should be used, based on reading the full skill file—not the name, a GitHub README, or memory alone. |
| **Tool and skill availability (honesty)** | If a Slot C token is **listed** but **not** present (no MCP, no plugin, no CLI), or a Slot D skill is **not installed / not fully read / not relevant**, omit that optional slot, **do not** claim the tool or skill ran, and record the first omission plus any material status changes in the ledger—then continue with available substitutes until normal stop rules. Owning **prompt-upgrade** does **not** magically install integrations or skills; the **Snapshot** / environment wins. |
...
Follow **this file top-to-bottom** on first read. **Do not read Lexicon before NON-NEGOTIABLE EXECUTION GATES.** For **`improvement_mode` = thorough**, **deep-upgrade intent**, or non-trivial **skill-targeting** edits, this read order is mandatory before **Implement**; do not use trivial / single-axis skip paths. Load [reference-execution-gates.md](reference-execution-gates.md) before Gate 1 or Gate 3 prompts. Load other companions by product class, risk surface, or stop-rule need—not all every time.

1. **Opening + NON-NEGOTIABLE EXECUTION GATES + PRE-FLIGHT** — pass/fail checklist; open [reference-execution-gates.md](reference-execution-gates.md) if Task patterns needed.
2. **Improvement mode** — identify mode from intent when clear; if product-making/changing and mode not identifiable, ask once and wait. **Deep-upgrade intent** pins **thorough** and tier obligations.
3. **Core idea** + **Execution contract** + **Common agent failure modes** — why gates exist; strict runtime enforcement.
4. **Trigger phrases** — route user intent to gates + mode.
5. **Lexicon** — parse terms (`theme_key`, subagent, execution gate, quintessence).
6. **Principle** + **Skill-first reasoning** + **Epistemic discipline** + **Professional voice** — honesty bar.
7. **Reference-informed coverage** + **P0 / P1 / P2** — target set; complement-bias.
8. **Stochastic Role Orchestra** through **Many roles → quintessence** — word banks, orchestra loop, integration.
9. **Workflow** steps 1–8 — **Implement (§5) requires Gates 2–3**; Verify feeds Gate 4.
10. **Companion table** → open **reference-execution-gates.md** when passing Gate 1 or 3; other **reference-*** by product class.
11. **Skill-targeting / Recursive self-application** — meta-work uses **same gates** before line edits.
...
**There is no fixed roster of job titles.** Instead you **field many subagents** by composing roles at runtime: each subagent gets a unique label from **Slot A** + **Slot B**, sampled from large, evolving word banks, and optionally **Slot C** when a verified tool/MCP/plugin should be used and **Slot D** when an installed, fully-read Agent Skill should guide the pass (see [reference-word-banks.md](reference-word-banks.md)). **Slot A** biases *how* they analyze (**risk posture**, **channel**—UI, API, data, ops, **depth**). **Slot B** biases *who* they are (title, archetype, specialist). **Slot C** (optional) names *which integrated tool surface* may run—**only** if the **Snapshot** shows it exists. **Slot D** (optional) names *which installed skill instruction package* may guide the pass—**only** after read-before-use and guideline requirements are met. Together A+B(+C)(+D) imply a **domain of strength**—the angle that subagent should stress hardest on that pass.
...
Names **which** MCP, plugin, CLI hook, or IDE integration **may** be invoked on that pass (from the **Slot C** bank in [reference-word-banks.md](reference-word-banks.md)). **Not** a persona. **Omit** entirely unless the **Snapshot** confirms availability—see **Tool and skill availability (honesty)** in **Lexicon** and the **Availability rule** in the reference file.

### Slot D — installed skill instruction (optional)

Names **which installed Cursor Agent Skill** may guide that pass (from the **Slot D** bank or a user-added exact skill name/path in [reference-word-banks.md](reference-word-banks.md)). **Not** a persona and **not** a callable tool. **Omit** entirely unless the **Snapshot** confirms the skill is installed, its full `SKILL.md` has been read in this session, a short **skill guideline** has been recorded, and the skill is relevant to the current pass. `prompt-upgrade` remains the governing orchestration contract; Slot D skills are managed subordinate instructions.

### Composed role label

Write the label as bracketed slots—minimum **two** groups, optional **third** for tools and optional **fourth** for installed skills—e.g. `[principal, privacy-first][sound designer]`, `[full-stack, skeptical][customer, linguist]`, `[visual-first][UI designer][Figma]` **only** when Figma (or Figma MCP) is actually available, or `[visual-first][UI designer][Figma][ui-ux-pro-max]` **only** when both the tool and installed skill pass availability rules. This label is the **persona + capability** baggage carried into that micro-pass (A/B persona; C tool hint; D skill-instruction hint).

### Fresh-agent constraint (no cross-persona memory)

For each composed role, simulate a **fresh agent**:

- Inputs allowed: current **product snapshot** (see **Lexicon**), constraints from the user, the **composed role label**, and—when **Slot C** is used—the **verified** list of tools/MCPs/plugins **actually** available (from environment / user confirmation); when **Slot D** is used, the read-derived **skill guideline** and relevant installed skill instructions. If a named tool is **not** available, **drop Slot C**; if a named skill is **not installed, not fully read, or not relevant**, **drop Slot D** and record the omission per **Tool and skill availability (honesty)**.
...
1. **Sample** N **distinct** composed roles from the **word banks** (Slot A + Slot B; add Slot C only when justified **and** tools exist per **Snapshot**; add Slot D only when an installed skill is fully read, guideline-recorded, and relevant). **N** should be **as large as justified** by risk, ambiguity, surface area, and **`improvement_mode` = thorough`**—keep drawing **orthogonal** pairs while information gain remains; **stop** on wave rules, not on a comfortable minimum. When the task is narrow or **`improvement_mode` = normal** (lighter), **N** is smaller but still **> 1** unless the user constrained to a **single-axis** issue. Use **stochastic diversity** so consecutive draws are not synonyms.
...
**DO NOT enter Workflow step 5 (Implement)** until **all** of the following are true:

1. **Orchestra completed** — At least **one** auditable orchestra wave with **N ≥ 2** distinct composed labels for product-making work (`N ≥ 1` only when user explicitly scoped single-axis).
2. **Integrated quintessence exists** — Action register lists **accepted** `theme_key` rows with Work Filter justification—not undigested role notes.
3. **Ledger row for latest wave** — Current wave recorded **before** the first implementing edit (not deferred to closing paragraph).
4. **Deep-upgrade tier** (when pinned) — Obligations A–F attempted, skipped, or substituted **in the ledger**.
5. **Runtime honesty** — If Task/subagent tooling was available and unused, gate **blocks** until at least one runtime delegation runs **or** ledger explains simulated-only path.
6. **Read-order honesty** — For thorough/deep-upgrade or non-trivial skill-package work, the main contract was read in order and required Gate 1 / Gate 3 companion templates were loaded; depth-only companions are opened only when their surface applies.

**Allowed without this gate:** Understand, Research, read-only exploration, mode questions, ledger/orchestra setup. **Not allowed:** scaffold commits, “quick MVP” file creation, or “I’ll backfill the orchestra later.”

5. **Implement** — **Prerequisite: NON-NEGOTIABLE EXECUTION GATES §Gate 2–3 and Pre-implementation gate above.** **Self-audit (required):** Before the first implementing tool call, confirm in one line: `Orchestra gate: PASS | FAIL` citing wave count, role count, ledger location. If **FAIL**, do not implement—return to step 3. Only **quintessence** actions passing the Work Filter (integrated **`theme_key`** rows). **Gate 3:** launch **one runtime subagent per adopted `theme_key`** (or documented batch); each subagent gets a **fresh** composed role scoped to that row, using the runtime prompt invariant and Gate 3 templates in [reference-execution-gates.md](reference-execution-gates.md). **Do not** implement from raw orchestra output or parent-agent synthesis alone when Task/delegation is available. Preserve working parts unless replacement clearly wins. Prefer explicit labels, readable hierarchy, simple state flow, structured configs over brittle strings. When the artifact is **code**, prefer **minimal scoped diffs** that match existing naming, types, and layout conventions—churn for its own sake still fails the Work Filter.

6. **Verify** — **Gate 4 verification pointer.** Project-standard checks when they exist; otherwise honest manual verification and stated gaps. Exercise important flows; check language switching after copy changes; full-stack: API/DB/auth/errors/loading/deploy. **State only what was actually verified.** When **deep-upgrade intent** applies to an **experience-forward web app**, verification **must** include route smoke **and** Lighthouse-class audit **when the toolchain permits**—see **Deep-upgrade intent** tier **F**; otherwise record the omission, substitute verification, and residual gap in the ledger. For artifacts **without a meaningful toolchain** (static HTML/CSS/JS, notebooks, sketches, or other non-CI workflows), sanity-read critical paths, walk key flows when possible, and **name what was not exercised**—do not imply CI, tests, builds, Lighthouse, or route QA ran when they did not. For **thorough/deep-upgrade** runs or **non-trivial skill-package edits**, run a fresh independent acceptance / evaluation subagent after edits when tooling allows; if unavailable, record a simulated substitute and residual risk.

7. **Self-review (Reflection)** — Re-read changed areas and **challenge prior conclusions**: what blocks trust for a newcomer; what confuses a maintainer next month; what would embarrass at ship; do stated claims still match the **Snapshot**? For **thorough/deep-upgrade** runs or **non-trivial skill-package edits**, this reflection includes a fresh independent acceptance / evaluation subagent after edits when tooling allows. Re-orchestrate with **new** composed roles (**[Slot A][Slot B]** with optional **[Slot C]**) only if they could expose a **distinct** failure mode (not a rephrase of an earlier pass)—same Gate 1 label discipline. Score changed or adopted parts across relevant governance aspects; target **≥90/100 per part / dimension**. If a part scores below target and a new Work Filter-passing `theme_key` exists, repeat the subagent process. If no new evidence/theme appears, or stop rules / Work Filter reject further churn, stop honestly with residual risk.

8. **Stop** — When verification passes and remaining ideas are speculative, optional, oversized, or need product direction. Do not claim “optimized” if a fresh orthogonal role would still surface a clear, non-redundant gap. For orchestra-only work, also apply the **wave stop rules** in [reference-workflow-registers.md](reference-workflow-registers.md) (section *Stop rule for orchestra waves*: coverage saturation, no new `theme_key`s, or duplicate rephrasings—not endless role spam). **Outward closure:** when stopping here after shipped work for this scope, apply **Outward closure discipline** (Lexicon)—finalize without trailing engagement prompts unless exceptions in **Skill-first reasoning & outward closure** §2 apply. **Terminal reply for thorough/deep-upgrade:** must include **auditable wave ledger** (Gate 4)—not the first appearance of orchestra/ledger if Implement already ran.
...
## Tool and skill availability (honesty) — MCP, plugins, hooks, skills

**Using only `prompt-upgrade` does not grant integrations or skills.** Slot C in [reference-word-banks.md](reference-word-banks.md) lists **candidate** tools (Figma MCP, GitHub MCP, Browser MCP, Docker, …); whether any of them **runs** depends on the user’s Cursor config, installed plugins, MCP servers, `PATH`, and credentials. Slot D lists **candidate installed Agent Skills**; whether any of them may guide a pass depends on the active skill list / filesystem, full read-before-use, and relevance to the current task.
...
- **Before** attaching **`[Slot D]`**, verify the exact skill is installed, read its full `SKILL.md` in this session, and record a short skill guideline (`use for` + `when to use`). Installed is necessary, not sufficient; attach Slot D only when the guideline fits the current pass.
...
- If a workflow **would** benefit from a listed skill but it is missing, unread, or irrelevant, omit Slot D and state the skipped/downgraded path when material. **Never** invoke a skill from memory, a prior session, or a GitHub name alone.
- Users may **grow** the Slot C bank with their own MCP/plugin names and the Slot D bank with their own installed skill names/paths over time; names on disk **outrank** wishful defaults.

This skill’s goal is **one orchestration contract** that can **route** work through whatever the user **actually** has—**not** a guarantee that every seed token is callable or every skill name is installed. Slot C tools and Slot D skills are managed capabilities; `prompt-upgrade` remains the governing controller for gates, Work Filter, and stop rules.
